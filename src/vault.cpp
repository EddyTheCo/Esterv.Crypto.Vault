...
Vault::Vault(QObject *parent, const QString filename)
    : QObject(parent), m_ctx(EVP_CIPHER_CTX_new()), m_cipherText(128, Qt::Initialization::Uninitialized),
      m_file(filename), m_isEmpty(true)
{
    readFromFile();
...
{
    if (m_isEmpty || password.size() < 8)
        return false;

    auto passHash =
        QPasswordDigestor::deriveKeyPbkdf2(QCryptographicHash::Sha512, password.toUtf8(), m_iv, NITERATIONS, 64);
...
bool Vault::changePassword(QString oldPass, QString newPass)
{
    ...
    if (m_isEmpty || newPass.size() < 8 || oldPass.size() < 8)
        return false;

    auto oldPassHash =
        QPasswordDigestor::deriveKeyPbkdf2(QCryptographicHash::Sha512, oldPass.toUtf8(), m_iv, NITERATIONS, 64);

    if (oldPassHash == m_passHash)
    {
        auto oldkey =
            QPasswordDigestor::deriveKeyPbkdf2(QCryptographicHash::Sha256, oldPass.toUtf8(), m_iv, NITERATIONS, 32);

        const auto plainText = getContent(oldkey);
        setRandomIV();
        auto newPassHash =
            QPasswordDigestor::deriveKeyPbkdf2(QCryptographicHash::Sha512, newPass.toUtf8(), m_iv, NITERATIONS, 64);

        auto newkey =
            QPasswordDigestor::deriveKeyPbkdf2(QCryptographicHash::Sha256, newPass.toUtf8(), m_iv, NITERATIONS, 32);
...
bool Vault::setContent(QByteArray plainText, QByteArray key)
{
    ...
    if (1 != EVP_EncryptInit_ex(m_ctx, EVP_aes_256_cbc(), NULL, reinterpret_cast<const unsigned char *>(key.data()),
                                reinterpret_cast<const unsigned char *>(m_iv.data())))
        return false;

    if (1 != EVP_EncryptUpdate(m_ctx, reinterpret_cast<unsigned char *>(var_cipherText.data()), &len,
...
{
    auto passHash = QPasswordDigestor::deriveKeyPbkdf2(QCryptographicHash::Sha512, password, m_iv, NITERATIONS, 64);

    if (!m_passHash.isEmpty() && passHash == m_passHash)
    {
        auto key = QPasswordDigestor::deriveKeyPbkdf2(QCryptographicHash::Sha256, password, m_iv, NITERATIONS, 32);
...
QByteArray Vault::getContent(QByteArray key) const
{
    ...
    if (1 != EVP_DecryptInit_ex(m_ctx, EVP_aes_256_cbc(), NULL, reinterpret_cast<const unsigned char *>(key.data()),
                                reinterpret_cast<const unsigned char *>(m_iv.data())))
        return QByteArray();

    if (1 != EVP_DecryptUpdate(m_ctx, reinterpret_cast<unsigned char *>(plainText.data()), &len,
                               reinterpret_cast<const unsigned char *>(m_cipherText.data()), m_cipherText.size()))
        return QByteArray();
    plaintext_len = len;
